...
document.addEventListener('DOMContentLoaded', () => {
    ...
    canvas.style.background = 'var(--bg-dark-blue)';

    let particlesArray;

    // Background image setup (moving background)
    const bgImage = new Image();
    bgImage.src = 'background.jpg'; // Place your image at project root or adjust the path
    let bgPattern = null;
    let bgOffsetX = 0;
    let bgOffsetY = 0;
    const bgSpeedX = 0.12; // Adjust for desired horizontal movement speed
    const bgSpeedY = 0.06; // Adjust for desired vertical movement speed
    ...
    function animate() {
        ...
        // Draw moving background image or solid dark blue fallback
        if (bgPattern && bgImage.width && bgImage.height) {
            bgOffsetX = (bgOffsetX + bgSpeedX) % bgImage.width;
            bgOffsetY = (bgOffsetY + bgSpeedY) % bgImage.height;
            ctx.save();
            ctx.translate(-bgOffsetX, -bgOffsetY);
            ctx.fillStyle = bgPattern;
            ctx.fillRect(0, 0, canvas.width + bgImage.width, canvas.height + bgImage.height);
            ctx.restore();
        } else {
            const styles = getComputedStyle(document.documentElement);
            const fallback = styles.getPropertyValue('--bg-dark-blue') || '#0b2a5b';
            ctx.fillStyle = fallback.trim();
